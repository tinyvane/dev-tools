...
- `codesync sync` 的 auto-clone 若发现目标目录已存在、损坏、归属冲突或 clone 失败，
  publish 阶段会显式排除同一路径；不再把刚刚拒绝覆盖的目录又当成本地孤儿发布，
  也不会出现重复的发布倒计时。
- 严格识别“clone 目标的唯一子目录才是同 owner/name 仓库”的多嵌套布局，输出实际 repo
  和可回退的布局整理命令，不再给出错误的“可作为孤儿发布”建议。

### Safety

- 嵌套识别要求父目录只有一个子项、子项是完整 Git 仓库且 GitHub origin 的 owner/name
  全部匹配；Codesync 只诊断和阻断本轮 clone/publish，不自动移动或删除用户目录。
