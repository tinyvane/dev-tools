...
- regex + 手写括号匹配解析 `$CodeRoots`、`$AutoClone`（V1 的 `$DbSyncTargets` **不再解析**，
  DB sync 已于 v2.13.0 移除）
...
- **不做 DB / 数据库同步**（v2.13.0 移除了 V1 继承来的 Docker MySQL sync）。原因：
  ...
  codesync 定位收敛为**纯 git repo 同步**。要 DB 同步请用专门工具。别再加回来。
