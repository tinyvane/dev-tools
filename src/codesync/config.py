...
class CodeRootProblem:
    """One unusable ``code_roots`` entry found before repo operations start."""
...
def code_root_problems(cfg: Config) -> list[CodeRootProblem]:
    """Return every unusable code root without creating or modifying anything.

    A missing mount or moved directory must not quietly become an empty repo
    scan: write-capable sync could otherwise interpret that absence as deletion
    intent. Even read-only commands fail so scripts cannot mistake incomplete
    output for a healthy empty configuration.
    """
    if not cfg.code_roots:
        return [CodeRootProblem(None, None, "没有配置任何 code_roots")]

    problems: list[CodeRootProblem] = []
    for configured in cfg.code_roots:
        ...
        try:
            ...
            problems.append(CodeRootProblem(configured, expanded, "目录不存在"))
        except (OSError, ValueError) as exc:
            problems.append(
                CodeRootProblem(configured, expanded, f"目录不可访问: {exc}")
            )
        else:
            if not stat.S_ISDIR(mode):
                problems.append(CodeRootProblem(configured, expanded, "路径不是目录"))
    return problems
...
def _to_toml(cfg: Config) -> str:
    lines: list[str] = []
    lines.append("# codesync config — generated by codesync.\n")
    ...
    if cfg.auto_clone:
        ...
        lines.append(
            "abort_if_local_missing_pct = "
            f"{ac.abort_if_local_missing_pct}"
        )
    ...
    if cfg.update:
        u = cfg.update
        lines.append("[update]")
        lines.append(f"check             = {'true' if u.check else 'false'}")
        lines.append(
            f"block_if_outdated = {'true' if u.block_if_outdated else 'false'}"
        )
        lines.append(f"ttl_hours         = {u.ttl_hours}")
        lines.append("")
...
def save(cfg: Config, *, backup: bool = False) -> Path | None:
    """Atomically save a complete config, optionally preserving its old bytes.

    The numbered backup is created before the temporary file replaces the live
    config. A failed write therefore leaves the original config usable.
    """
    ...
    if backup and target.exists():
        ...
        while backup_path.exists():
            backup_path = target.with_name(f"{target.name}.bak.{suffix}")
            suffix += 1
